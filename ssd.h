...
/* Storage type for hybrid SSD */
enum { STORAGE_TYPE_SLC = 0, STORAGE_TYPE_QLC = 1, MAX_STORAGE_TYPES };

/* Page access type for hotness tracking */
enum { ACCESS_READ = 0, ACCESS_WRITE = 1, ACCESS_UPDATE = 2 };
...
struct ssdparams {
	...
	/* Hybrid storage parameters */
	int storage_type; /* SLC or QLC */
	int slc_ratio; /* SLC capacity ratio (percentage) */
	int slc_channels; /* Number of channels allocated to SLC */
	int slc_luns_per_ch; /* Number of LUNs per channel for SLC */
	int qlc_channels; /* Number of channels allocated to QLC */
	int qlc_luns_per_ch; /* Number of LUNs per channel for QLC */
	
	/* Hotness tracking parameters */
	int hotness_table_size; /* Size of hotness tracking table */
	int hot_threshold; /* Access count threshold for hot pages */
	int cold_threshold; /* Access count threshold for cold pages */
	int migration_interval; /* Migration check interval (ns) */
	int max_migrations_per_check; /* Max migrations per check */
	...
	int qlc_pgs_per_blk;
	int qlc_blks_per_pl;
	int qlc_pgs_per_oneshotpg;
	int qlc_oneshotpgs_per_blk;
	...
	/* Simplified latency parameters */
	int slc_read_latency;  /* SLC read latency in nanoseconds */
	int slc_write_latency; /* SLC write latency in nanoseconds */
	int slc_erase_latency; /* SLC erase latency in nanoseconds */
	
	/* QLC latency parameters for 4 regions (Q1, Q2, Q3, Q4) */
	int qlc_q1_read_latency;  /* Q1 region read latency */
	int qlc_q2_read_latency;  /* Q2 region read latency */
	int qlc_q3_read_latency;  /* Q3 region read latency */
	int qlc_q4_read_latency;  /* Q4 region read latency */
	int qlc_write_latency;    /* QLC write latency (for migration) */
	int qlc_erase_latency;    /* QLC erase latency */
...
//66f1
	...
	/* Hybrid storage calculated values */
	unsigned long slc_tt_pgs; /* total # of pages in SLC */
	unsigned long slc_tt_blks; /* total # of blocks in SLC */
	unsigned long slc_tt_lines; /* total # of lines in SLC */
	unsigned long slc_pgs_per_ch; /* # of pages per channel in SLC */
	unsigned long slc_blks_per_ch; /* # of blocks per channel in SLC */
	
	unsigned long qlc_tt_pgs; /* total # of pages in QLC */
	unsigned long qlc_tt_blks; /* total # of blocks in QLC */
	unsigned long qlc_tt_lines; /* total # of lines in QLC */
	unsigned long qlc_pgs_per_ch; /* # of pages per channel in QLC */
	unsigned long qlc_blks_per_ch; /* # of blocks per channel in QLC */
...
/* Get storage type (SLC or QLC) based on PPA */
static inline uint32_t get_storage_type(struct ssd *ssd, struct ppa *ppa)
{
	struct ssdparams *spp = &ssd->sp;
	
	#if (BASE_SSD == HYBRID_SSD)
	/* Check if PPA is in SLC or QLC range */
	if (ppa->ppa < spp->slc_tt_pgs) {
		return STORAGE_TYPE_SLC;
	} else {
		return STORAGE_TYPE_QLC;
	}
	#else
	return STORAGE_TYPE_SLC; /* Default to SLC for non-hybrid */
	#endif
}

/* Get storage type from LPN */
static inline uint32_t get_storage_type_from_lpn(struct ssd *ssd, uint64_t lpn)
{
	struct ssdparams *spp = &ssd->sp;
	
	#if (BASE_SSD == HYBRID_SSD)
	/* Check if LPN is in SLC or QLC range */
	if (lpn < spp->slc_tt_pgs) {
		return STORAGE_TYPE_SLC;
	} else {
		return STORAGE_TYPE_QLC;
	}
...
/* Get QLC region (Q1, Q2, Q3, Q4) based on page number */
static inline uint32_t get_qlc_region(struct ssd *ssd, struct ppa *ppa)
{
	struct ssdparams *spp = &ssd->sp;
	uint64_t qlc_page_offset;

	#if (BASE_SSD == HYBRID_SSD)
	/* Calculate offset within QLC region */
	qlc_page_offset = ppa->ppa - spp->slc_tt_pgs;

	/* Divide QLC region into 4 equal parts */
	uint64_t region_size = spp->qlc_tt_pgs / 4;

	if (qlc_page_offset < region_size) {
		return 0;  /* Q1 */
	} else if (qlc_page_offset < region_size * 2) {
		return 1;  /* Q2 */
	} else if (qlc_page_offset < region_size * 3) {
		return 2;  /* Q3 */
	} else {
		return 3;  /* Q4 */
	}
	#else
	return 0;  /* Default to Q1 for non-hybrid */
	#endif
}
...
/* Hotness tracking structure */
struct page_hotness {
	uint64_t lpn;
	uint32_t access_count;     /* Total access count */
	uint32_t recent_access;    /* Recent access count (for aging) */
	uint64_t last_access_time; /* Last access timestamp */
	uint32_t storage_type;     /* Current storage type (SLC/QLC) */
	bool is_migrating;         /* Whether page is being migrated */
};

/* Migration management structure */
struct migration_mgmt {
	struct page_hotness *hotness_table; /* Hotness tracking table */
	uint64_t hotness_table_size;        /* Size of hotness table */
	uint32_t hot_threshold;             /* Threshold for hot pages */
	uint32_t cold_threshold;            /* Threshold for cold pages */
	uint64_t migration_interval;        /* Migration check interval */
	uint64_t last_migration_check;      /* Last migration check time */
	uint32_t max_migrations_per_check;  /* Max migrations per check */
	uint32_t current_migrations;        /* Current migration count */
};
